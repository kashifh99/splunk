...
if app is None:
    app = raw_input('Please enter the app that contains the search(es): ')
...
if len(ssname_list) == 0:
  while True:
    n = raw_input("Please enter the name of saved search #%d (empty value to stop entering): " % (len(ssname_list)+1))
    if len(n) == 0:
      break
    ssname_list.append(n)
    
if sk is None and  user is None:
  user = raw_input('Please enter your splunk username: ')
if sk is None and password is None:
  password = getpass.getpass('Please enter your splunk password: ')
if et is None:
  et = raw_input('Please enter the earliest time (UTC or relative): ')
if lt is None:
  lt = raw_input('Please enter the latest time (UTC or relative): ')
