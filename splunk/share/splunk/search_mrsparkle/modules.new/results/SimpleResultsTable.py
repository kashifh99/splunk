    ...
    def generateResults(self, host_app, client_app, sid, count=1000,
        ...
        for field in fieldNames:
            output.append('<th class="sorts %s"><a href="#"><span class="sortLabel">%s</span> <span class="splSort%s"></span></a></th>' \
                % (field != sortField and "None" or sortDir, cgi.escape(field, quote=True), field != sortField and "None" or sortDir))
        ...
        for i, result in enumerate(dataset):
            ...
            for field in fieldNames:
                ...
                if field=='_time' and result.time:
                    ...
                    output.append(' field="%s">' % cgi.escape(field, quote=True))
                    output.append(cgi.escape(fieldValues.getRaw(), quote=True));
                    output.append('</td>')
                ...
                # cap display count to prevent blowout
                    ...
                    if isSparklines:
                        renderedValues = [cgi.escape(x.value, quote=True) for x in fieldValues[:MAX_SPARKLINE_MV_COUNT]]
                    else:
                        renderedValues = [cgi.escape(x.value, quote=True) for x in fieldValues[:MAX_MULTI_VALUE_COUNT]]
