...
Splunk.Module.AjaxInclude = $.klass(Splunk.Module, {
    ...
    initialize: function($super, container){
        ...
        this.focus = Splunk.util.escapeHtml(this._params.focus) || null;
        this.data = this._params.data || {};
        this.hrefRegExp = new RegExp(this._params.hrefpattern, this._params.hrefattributes || "");
        this.request(this._params.method, Splunk.util.make_url(encodeURI(this._params.endpoint)));
        this.data = {};
    ...
    applyFormFocus: function(){
        if(this.focus){
            try {
                ...
                this.logger.error("Invalid focus element " + this.focus);
            }
        }else{
            this.autoFormFocus();
        }
    ...
    request: function(method, endpoint){
        var BAD_URL = /\/splunkd\/__raw\/services\/authentication\//i;
        var shouldNotSend = BAD_URL.test(endpoint) && method != 'GET';
        if (!shouldNotSend) {
            $.ajax({
                   type: method,
                   url: endpoint,
