define(
    ...
    function($, _, SavedSearch, Viewstate, Cron, JobsCollection, HistoriesCollection, time_utils, splunkd_utils, general_utils, splunkUtil, i18n, route, URI_CONSTANTS, math_utils) {
        /**
         * @constructor
         * @memberOf models
         * @name Report
        ...
        var ReportModel = SavedSearch.extend({
            ...
            getScheduleString: function() {
                ...
                if (!this.isRealTime()) {
                    if (this.entry.content.get('is_scheduled')) {
                        cronModel = Cron.createFromCronString(this.entry.content.get('cron_schedule'));
                        text = cronModel.getScheduleString();
                    } else {
            ...
            getExportedURLParams: function(viewRouteData, data, timeParams) {
                if (viewRouteData && viewRouteData.route && viewRouteData.route().indexOf(URI_CONSTANTS.PAGES.ANALYSIS_WORKSPACE) >= 0) {
                    return $.extend({}, data, timeParams);
                }
                return data;
            }
        },
