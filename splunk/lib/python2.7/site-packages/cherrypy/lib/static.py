...
def render_index(path, branch):
    ...
    filelist = []
    if branch:
        filelist.append('<li><a href="../">Parent Directory</a></li>')
    for fn in files:
        fnquote = cgi.escape(fn.encode('utf-8'))
        if os.path.isdir(os.path.join(path, fn)):
            filelist.append('<li><a href="%s/">%s</a></li>' % (fnquote, fnquote))
        else:
            filelist.append('<li><a href="%s">%s</a></li>' % (fnquote, fnquote))
...
def staticdir(section, dir, root="", match="", content_types=None, index="", generate_indexes=False):
