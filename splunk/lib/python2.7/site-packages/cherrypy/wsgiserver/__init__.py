...
try:
    # Python 2.6 has SSL built in which doesn't cause Python to segfault.. which i kind of like.
...
class SSL26_fileobject(SSL_fileobject):
    def _safe_call(self, is_reader, call, *args, **kwargs):
        start = time.time()
        while True:
            try:
                return call(*args, **kwargs)
            except nativessl.SSL_ERROR_WANT_READ:
                # Sleep and try again. This is dangerous, because it means
                # the rest of the stack has no way of differentiating
                # between a "new handshake" error and "client dropped".
                # Note this isn't an endless loop: there's a timeout below.
                time.sleep(self.ssl_retry)
            except nativessl.SSL_ERROR_WANT_WRITE:
                time.sleep(self.ssl_retry)
            except nativessl.SSL_ERROR_EOF:
                if is_reader:
                    return ""
                ...
                if str(e) == "The read operation timed out":
                    ...
                    raise
            except (nativessl.SSL_ERROR_ZERO_RETURN, nativessl.SSL_ERROR_WANT_X509_LOOKUP, nativessl.SSL_ERROR_WANT_CONNECT, nativessl.SSL_ERROR_SSL, nativessl.SSL_ERROR_SYSCALL), e:
                raise FatalSSLAlert(*e.args)
            except:
                raise
...
class HTTPConnection(object):
    ...
    def __init__(self, sock, wsgi_app, environ, addr):
        self.socket = sock
        self.remote_ip_addr = addr
        self.wsgi_app = wsgi_app
        ...
        if nativessl and isinstance(sock, nativessl.SSLSocket):
            self.rfile = SSL26_fileobject(sock, "rb", self.rbufsize)
            self.wfile = SSL26_fileobject(sock, "wb", -1)
        elif SSL and isinstance(sock, SSL.ConnectionType):
    ...
    def communicate(self):
        ...
        sock = self.socket
        if nativessl and isinstance(sock, nativessl.SSLSocket):
            # we defer SSL handshaking until now to avoid blocking the accept loop
            timeout = sock.gettimeout()
            try:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    conn.close()
                    # check if concurrent connection limit is enforced
                    if(self.server.concur_limit_per_ip > 5 ):
                        # remove so that this ip address can avail one more connection.
                        # decrease count of concurrent connections from this ip address.
                        self.server._conns.remove(conn.remote_ip_addr)
                    self.conn = None
...
class ThreadPool(object):
    ...
    def start(self):
        ...
        for worker in self._threads:
            # SPL-50645 check splunkweb stop request 
            if self._stop:
                break
        ...
        self._monitor_thread = threading.Thread(target=self.monitor_threads)
        self._monitor_thread.start()
    
    def monitor_threads(self):
        """ Increase or reduce the pool of idle threads """
        min_spare = self.min_spare
        ...
        while not self._stop:
            ...
            if self.shutdown_threads > 0:
                # SPL-66357 / SPL-60287 remove dead threads
    ...
    def shrink(self, amount):
        ...
        # SPL-66357 number of worker threads should not drop below self.min
        if amount > 0:
            amount = min(amount, len(self._threads) - self.min - self.shutdown_threads)
...
try:
    ...
    try:
        ...
        def prevent_socket_inheritance(sock):
            ...
            fileno = sock.fileno()
            windll.kernel32.SetHandleInformation(fileno, 1, 0)
else:
    def prevent_socket_inheritance(sock):
...
class CherryPyWSGIServer(object):
    ...
    concur_limit_per_ip: Maximum number of concurrent connections allowed from one ip address. 
        Minimum enforced 6. 
        Value < 6, unlimited concurrent connections.
    ...
    def __init__(self, bind_addr, wsgi_app, numthreads=10, server_name=None,
                 max=-1, min_spare=5, max_spare=10, request_queue_size=5, 
                timeout=10, shutdown_timeout=5, concur_limit_per_ip=-1):
        self.requests = ThreadPool(self, min=numthreads or 1, max=max, 
            min_spare=min_spare, max_spare=max_spare)
        ...
        self.concur_limit_per_ip = concur_limit_per_ip 
    ...
    def start(self):
        ...
        # List to keep track of concurrent connections from different ip addresses
        self._conns = []
    ...
    def bind(self, family, type, proto=0):
        ...
        if self.ssl_certificate and self.ssl_private_key:
            if not (SSL or nativessl):
                raise ImportError("You must install pyOpenSSL to use HTTPS.")
            
            # See http://aspn.activestate.com/ASPN/Cookbook/Python/Recipe/442473
            if SSL: # prior to Python 2.6
                ctx = SSL.Context(SSL.SSLv23_METHOD)
                ctx.use_privatekey_file(self.ssl_private_key)
                ctx.use_certificate_file(self.ssl_certificate)
                self.socket = SSLConnection(ctx, self.socket)
            if nativessl:
                ctx = nativessl.SSLContext(self.ssl_version if self.ssl_version >= 0 else nativessl.PROTOCOL_SSLv23)
                ctx.options |= self.ssl_options
                if self.ssl_certificate:
                    ctx.load_cert_chain(self.ssl_certificate, self.ssl_private_key)
                if self.ssl_ciphers:
    ...
    def tick(self):
        ...
        try:
            ...
            conn = self.ConnectionClass(s, self.wsgi_app, environ, addr[0])
            # Minimum number of concurrent connections can not be too low. Otherwise dashboards will not work.
            # check if concurrent connections limit is set more than 5 and find how many concurrent connections from this ip address
            if(self.concur_limit_per_ip > 5 and len([t for t in self._conns if t == addr[0]]) < self.concur_limit_per_ip):
                # Allow this connection as below concurrent connection limit
                self._conns.append(addr[0])
                self.requests.put(conn)
            else:
                if(self.concur_limit_per_ip < 6 ):
                    #unlimited concurrent connection
                    self.requests.put(conn)
                else:
                    # Exceeded concurrent connection limit. Do now allow this connection
                    conn.close()
