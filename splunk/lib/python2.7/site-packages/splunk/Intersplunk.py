...
def getKeywordsAndOptions(charset=None):
    ...
    for arg in argv:
        ...
        if len(matches) == 0:
            if charset!=None and isinstance(arg,str):
            ...
            for match in matches:
                attr, eq, val = match
                # put arg in a match
                if charset!=None and isinstance(val,str):
...
'''            
            ...
            results = readResults(StringIO(input.encode('UTF-8')))
            ...
            outputResults(results, outputfile=sys.stdout)
            generatedOutput = sys.stdout.getvalue()
            generatedOutput = generatedOutput.replace('\r\n', '\n')
            if MV_ENABLED:
                self.assertEqual(generatedOutput.decode('UTF-8'), expectedOutput)
  
            sys.stdout.truncate(0) 
            outputResults(results, fields=['constant','sourcetype'], outputfile=sys.stdout)
            generatedOutput = sys.stdout.getvalue()
            generatedOutput = generatedOutput.replace('\r\n', '\n')
            if MV_ENABLED:
