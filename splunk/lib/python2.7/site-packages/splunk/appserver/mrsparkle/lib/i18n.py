...
import cherrypy
import babel, babel.support, babel.numbers
from babel.messages.pofile import unescape
import os, os.path, gettext, copy, shutil, hashlib, urllib
from datetime import datetime, timedelta
import math
from splunk.appserver.mrsparkle.lib import util, apps
...
ISO8609_MICROTIME='%Y-%m-%dT%H:%M:%S.%Q%z'
...
def current_lang(as_string=False):
    """
...
def current_lang_url_component():
    """
    ...
    def __call__(self, path_info):
        ...
        if locale == 'en_DEBUG':
            ...
            if not cherrypy.request.lang:
                ...
                target = "%s?%s" % (path_info, urllib.quote(request.query_string, '&%=+#')) if len(request.query_string) else path_info
...
def get_translation(domain, languages):
    ...
    for lang in nelangs:
        ...
        for appname, app in apps.local_apps.items():
...
def get_all_translations(locale=None):
    ...
    """ 
    result = {'catalog': {}}
    result['plural'] = util.JSONLiteral(c2js(get_plural_rule(locale)))
...
def get_all_translations_cached(locale=None, autoload=False):
    ...
    if autoload:
        output = 'i18n_register(%s)' % json.dumps(catalog_contents)
    else:
        output = json.dumps(catalog_contents)
...
def init_i18n_cache(flush_files=False):
    ...
    try:
        f = file(os.path.join(CACHE_PATH, 'version'), 'r')
        current = str(f.read())
        f.close()
    ...
    if flush_files or build_number != current:
        ...
        f = file(os.path.join(CACHE_PATH, 'version'), 'w')
...
def translate_js(filename, locale=None):
    if not locale:
        locale = cherrypy.request.lang
    cache_file = os.path.join(CACHE_PATH, "%s-%s-%s.cache" % (os.path.basename(filename), hashlib.sha1(os.path.abspath(filename)).hexdigest(), locale))
    ...
    shutil.copyfileobj(file(filename, 'rb', 128*1024), f)
    f.close()
    try:
        if os.path.exists(cache_file):
...
def generate_js(filename, locale=None):
    ...
    trans_table['plural'] = util.JSONLiteral(c2js(trans_table['plural']))
...
def dispatch_i18n_js(filename):
    ...
    cache_file = os.path.join(CACHE_PATH, "%s-%s-%s.cache" % (os.path.basename(filename), hashlib.sha1(os.path.abspath(filename)).hexdigest(), locale_name))
    ...
    shutil.copyfileobj(file(filename, 'rb'), f)
...
def write_moment_translation(js_path, locale, dest_file):
    ...
    while len(lang_parts):
        ...
        if os.path.exists(lang_path):
            ...
            src = file(lang_path, 'rb', 128*1024)
    ...
    for src in sources:
        ...
        if s['type'] == 't':
            ...
            for el in root.xpath(xpathQuery):
                ...
                text = text + ''.join(et.tostring(c) for c in el)
...
class ViewExtractor(object):
    ...
    def extract_view(self, fileobj):
        import viewconf
        nativeObject = viewconf.loads(fileobj.read(), fileobj.name)
            ...
            if spec[0] == '*':
                ...
                if spec[0] in param:
                    for v in  self.extract_params(spec[1:], param[spec[0]]):
                        yield v
        elif not spec and isinstance(param, basestring):
