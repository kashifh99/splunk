...
import json
import logging
from splunk.util import isRedirectSafe, sanitizeBreadcrumbs
...
class StateDict(dict):
    ...
    def unserialize(cls, data, format=BASE64):
        ...
        if format==BASE64:
                try:
                    state = json.loads(zlib.decompress(base64.urlsafe_b64decode(str(data))))

                    if state.get('breadcrumbs'):
                        state['breadcrumbs'] = sanitizeBreadcrumbs(state['breadcrumbs'])

                    if state.get('return_to') and not isRedirectSafe(str(state['return_to'])):
                        del state['return_to']

                    if state.get('return_to_success') and not isRedirectSafe(str(state['return_to_success'])):
