...
import os
import splunk
import splunk.models.legacy_views.dashboard as libdash
import splunk.models.legacy_views.form as libform
from splunk.models.view_escaping.fromdash import createDashboardFromXml
import cherrypy
    ...
    for k in sorted(ROOT_VIEW_ATTRIBUTES):
        if k in obj:
            if obj[k] in (True, False):
                root.set(k, unicode(obj[k]).lower())
            elif obj[k] in (None, ''):
                continue
            else:
                root.set(k, unicode(obj[k]))
...
def loadLegacy(xmlString, viewName, viewDigest=None, sourceApp=None):
    ...
    xmlString = xmlString.encode('ascii', 'xmlcharrefreplace')
    root = safe_lxml.fromstring(xmlString, parser=parser)
    model = createDashboardFromXml(root, viewName=viewName, digest=viewDigest, sourceApp=sourceApp)
