import threading
import os.path
import StringIO # NOT cStringIO - research cStringIO unicode issues for why
import shutil
import hashlib
import traceback
import subprocess
...
import lib.i18n 
import cssmin
from lib.util import make_absolute, Popen
import splunk.util
import logging
import startup
...
MODULE_STATIC_CACHE_PATH = make_absolute(os.path.join('var', 'run', 'splunk', 'appserver' ,'modules', 'static'))

MRSPARKLE = make_absolute(os.path.join('share', 'splunk', 'search_mrsparkle'))

PATH_TO_JSMIN = make_absolute(os.path.join('bin', 'jsmin'))
...
def chain_common_js():
    ...
    with _chain_common_js_lock:
        ...
        try:
            ...
            cache_filename = make_absolute(os.path.join(lib.i18n.CACHE_PATH, '%s-%s-%s.cache' % ('common.min.js', hashlib.sha1('common.min.js').hexdigest(), locale)))
            ...
            for js_filename in js_filenames:
                if js_filename == 'i18n.js':
                    path = lib.i18n.dispatch_i18n_js(os.path.join(js_root, 'i18n.js'))
                    input_temp_fh = file(path, 'rb')
                    js += input_temp_fh.read() + '\n'
                    input_temp_fh.close()
                else:
                    path = os.path.join(js_root, js_filename)
                    if os.sep + 'contrib' + os.sep not in path:
                        block = lib.i18n.generate_wrapped_js(path, locale)
                        if block and block not in blocks:
                            blocks.append(block)
                            js += block + '\n'
                    input_temp_fh = file(path, 'r')
                    js += input_temp_fh.read() + ';\n'
                    input_temp_fh.close()
...
def generate_file_list_hash(l):
    return hashlib.sha1(''.join(l)).hexdigest()
...
def clear_cache():
    ...
    cache_paths = [
        (make_absolute(lib.i18n.CACHE_PATH), MODULE_JS_FILE_PREFIX),
        (make_absolute(os.path.join(MODULE_STATIC_CACHE_PATH, 'css')), MODULE_CSS_FILE_PREFIX)
    ]
