...
def getTimeRanges(namespace=None):
    ...
    if (subMenusPresent.keys() != subMenusDefinedByChildren.keys()) :
        logger.error("Configuration error in times.conf.  For each sub_menu key (%s) there must be an existing stanza (%s) and vice versa." % (subMenusDefinedByChildren.keys(),  subMenusPresent.keys()))
    ...
    # orderedStanzas.sort(cmp=lambda a,b: cmp(a['order'], b['order']))
    orderedStanzas.sort(compareTimeRanges)
    return orderedStanzas
