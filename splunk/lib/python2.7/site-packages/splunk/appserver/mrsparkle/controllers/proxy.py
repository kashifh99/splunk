...
def precompile_whitelist():
    ...
    whitelist = []
    for stanzaname in webconf.keys():
        ...
        newelem['methods'] = filter(len, map(lambda x: x.strip(), stanza.get('methods', 'GET').upper().split(',')))
...
class ProxyController(BaseController):
    ...
    def index(self, oid=None, **args):
        ...
        isFileUpload = isinstance(fs, cgi.FieldStorage) and fs.file
