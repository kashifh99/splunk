...
class AdminController(BaseController, Capabilities):
    ...
    def saveEntity(self, endpoint_path, entity_name, args, namespace, entity_owner, entity_uri):
        ...
        if endpoint_path.startswith('deployment/server/setup/data/inputs/remote_monitor'):
            ...
            entity_uri += '?' + urllib.urlencode(appName)    
    ...
    def permissions(self, namespace, action, endpoint_path, sharing='user', owner=None, perms_read=None, perms_write=None, **kwargs):
        ...
        if entity['eai:acl'].get('sharing') == 'app' or entity['eai:acl'].get('can_share_app', None) == '1':
            if not util.isLite():
                app_label = _('This app only')
                sharing_options.append(('app', app_label+' ('+entity_name+')'))
    ...
    def multiaction(self, namespace, endpoint_base, multiaction, **kwargs):
        ...
        if endpoint_base.startswith('data/props/extractions'):
            qs = {'safe_encoding' : 1}
            control_link += '?' + urllib.urlencode(qs)
        ...
        if control_link.startswith('/services/deployment/server/setup/data/inputs/'):
            ...
            postargs = {}
            serverClass = '*'
            qs = urlparse.urlsplit(control_link).query
            if len(qs) > 0:
                qs_args = urlparse.parse_qs(qs)
    ...
    def datainputstats(self, namespace, action, operation=None, **kwargs):
        ...
        try:
            ...
            for (key, val) in modInputTypes.items():
                ...
                if key in builtin_mod_inputs.keys():
    ...
    def builtin_mod_inputs(self):
        ...
        add_mod_input('MonitorNoHandle', ignore = True)
        # This input is deprecated. We use the win-event-log-collections
        add_mod_input('WinEventLog', ignore = True)
        add_mod_input('WinHostMon', input_type = 'hostmon')
        add_mod_input('WinNetMon', input_type = 'netmon')
        add_mod_input('WinPrintMon', input_type = 'printmon')
        # The WinRegMon is deprecated. We use the `registry` input.
        add_mod_input('WinRegMon', ignore = True)
        # The admon modinput is deprecated. We use the `ad` input.
        add_mod_input('admon', ignore = True)
        # No manager xml for this input. Viewable by all users.
        add_mod_input('powershell', has_manager_xml = False, add_as_mod_input = True)
        # The powershell2 is deprecated. We use the `powershell` input above.
        add_mod_input('powershell2', ignore = True)
        # This input is deprecated. We use the `win-perfmon` input.
        add_mod_input('perfmon', ignore = True)
    ...
    def accesscontrols(self, namespace, action, operation=None, **kwargs):
        if not self.can_access_manager_xml('accesscontrols'):
            logger.error('Cannot reach "accesscontrols". Insufficient user permissions.')
            raise cherrypy.HTTPError(404)

        # if accesscontrols is available, then at least one of the below must be true...
        can_authoverview = self.can_access_manager_xml('authoverview')
        can_users = self.can_access_manager_xml('authentication/users')
        can_roles = self.can_access_manager_xml('authorization/roles')
        can_password_management = self.can_access_manager_xml('password/management')

        return self.render_admin_template('admin/accesscontrols.html', {
            'namespace'              : namespace,
            'can_authoverview'       : can_authoverview,
            'can_users'              : can_users,
            'can_roles'              : can_roles,
            'can_password_management': can_password_management,
    ...
    def _prepareEntries(self, entries):
        ...
        for e in entries:
            ...
            if e['appID'] in sbIdsOfLocalApps:
                ...
                updateLink = filter((lambda x: x[0] == 'update'), en_local[localAppId].links)
                if updateLink:
