...
class FieldController(BaseController):
    ...
    def summary(self, action, sid, field_name, count=None, **args):
        ...
        if field_name is None:
            ...
            field_primarily_numeric = field_summary.get('numericCount')>field_summary.get('count')/2
            numerical_reports = [
    ...
    def field_actions(self, actions, namespace, sid, offset, field_name=None, field_value=None, latest_time=None, view=None, **args):
        ...
        # if we have globbers
        event_field_names_list = event.fields.keys()
        ...
        for event_field_name in event_field_names_list:
            ...
            elif type(event_field) == splunk.search.ResultField and len(event_field) > 1:
                event_field_values[event_field_name] = unicode(event_field[0])

            else:
                event_field_values[event_field_name] = unicode(event_field)
            ...
            if fa_eventtypes != None:
                ...
                if len(fa_eventtypes) > 0:
                    if EVENTTYPE_FIELD_NAME not in event_field_names:
                        ...
                        event_eventtypes = set(splunk.util.stringToFieldList(unicode(event.fields[EVENTTYPE_FIELD_NAME])))
