...
import socket
import base64
from urlparse import urlparse, urlsplit, unquote
...
def dispatch(handlerClassName, requestInfo, sessionKey):
    ...
    org_stdout, sys.stdout = sys.stdout, file(os.devnull, 'wb')
    org_stderr, sys.stderr = sys.stderr, file(os.devnull, 'wb')
    try:
        try:
...
class HTTPResponse(object):
    ...
    def toXml(self):
        ...
        xml.append('</headers>')
        xml.append('<encodedpayload>%s</encodedpayload>' % base64.b64encode(''.join(self.responseBuffer)))
        xml.append('</response>')
...
class BaseRestHandler(object):
    ...
    def __init__(self, method, requestInfo, responseInfo, sessionKey):
        ...
        self.pathParts = map(unquote, requestInfo['path'].strip('/').split('/'))
        self.messages = []
...
def simpleRequest(path, sessionKey=None, getargs=None, postargs=None, method='GET', raiseAllErrors=False,
    ...
    if serverResponse.status == 401:
        ...
        if serverContent.count('forced_password_change') or serverContent.count('inval_pass_min_len'):
