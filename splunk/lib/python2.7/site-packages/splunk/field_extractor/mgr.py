...
def getAppsList(sessionKey, owner):
   return splunk.entity.getEntities('apps/local', search='visible=1 AND disabled=0', namespace=None, owner=owner, count=-1, sessionKey=sessionKey).keys()
...
def getIndexList(sessionKey, owner):
   ...
   if len(indexes) == 0:
      indexes = []
       ...
       defaultIndex = indexes.values()[0]['defaultDatabase'] # all values have duplicate data -- the default index
...
def getSourceFields(events):
       ...
       fieldsAndStats = fieldInfo.items()
       # sort most interesting fields first
       fieldsAndStats.sort(lambda x,y: cmp((100.0 * float(y[1][1]) * y[1][0]), 100.0 * (float(x[1][1]) * x[1][0]) ))
