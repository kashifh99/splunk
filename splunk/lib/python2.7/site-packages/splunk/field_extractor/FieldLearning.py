...
MAX_RULE_LEN = 300
# if we generate a regex with more than 100 elements, give up rule
MAX_REGEX_ELEMENTS = 1000 #300 #30
...
def _validateRules(events, examples, counterexamples, rules):
    ...
    for r in rules:
        #print("%s %s %s %s" % (str(r.getMatchCount()), str(r.getScore()), str(r._examplesCount), str(r)))
        ...
        for rule in rules:
            ...
            if len(extractions) > 0:
                for extraction in extractions:
                    #print("EXTRACTED: %s RULE: %s" % (extraction, rule))
...
def generateSimpleRegexes(text, isPrefix, multiline, loosenCount = False):
    ...
    for bestpos in bestpositions:
        ...
        try:
            ...
            regexes.append(regex)
            # NGHI SPL-91728
            simplifiedRegex = re.sub(r'(\\w\+\\s\+)+', r'(\w+\s+)+', regex) # reduce repetitions of \w+\s+ sequence
...
def getValueRegex2(extraction, forward, suffix, max_value_regex=0): #MAX_VALUE_REGEX):
    ...
    # make value extractions
    try:
        valueRegex = (splitText(extraction),1.0)
    except:
        valueRegex = (".+?",1.0)

    valueRegexes.append(valueRegex)

    # if forward rule, and there is not suffix (we're at the end
    # of the line), the value is just everything (.*)
    if forward and suffix == "":
        valueRegexes.append((".+",0.8))
    # if we have a suffix and the value regex is too complicated (too long)
    # try to simplify the value regex if it can be via the suffix
    elif suffix != "" and len(valueRegex[0]) > max_value_regex:
        endchar = suffix[0]
        # interesting suffix
        if endchar in VALUEPOST_CHARACTERS:
            # and  not in the extraction or is in extraction, but 
...
class PositionalRule:
    ...
    def calcScore(self):
        ...
        if self.learnedConsistent():
            ...
            #print("%f %s %u %u\n" % (score, self.getWholePattern(), goodCount, self.getLearnedCount()))
            #print("%s RULE: %s  goodcount %s examples %s consistency %s examples: %s learned: %s" % (score, self.getWholePattern(), goodCount, exampleCount, self.learnedConsistent(), self.getExamplesCount(), self.getLearnedCount()))
        #sys.stdout.write("SCORE %s: %s\t\tverperc %s learned %s example %s regexsize %s goodcount %s bias %s" % (score, self.getWholePattern(), exampleVarietyPerc, learnedCount, exampleCount, regexSize, goodCount, self._bias))
