...
def rule_to_str(rule):
    ''' rule is a dict. Example: 
        {'regex': '[^\\:]+',
         'type': 'any character except <i>:</i>'}
        This function converts the dict's unicode values into utf8 strings
    '''
    return {k: to_str(rule[k]) for k in rule}


def result_to_str(result):
    ''' Convert the result of TableUINewRule and TableUIExistingRule to utf8.
        result is a dict with keys 'start_rules', 'extract_rules', and 'stop_rules'.
    '''
    if 'start_rules' not in result:
        return
    return {
            'start_rules': map(rule_to_str, result['start_rules']),
            'extract_rules': map(rule_to_str, result['extract_rules']),
            'stop_rules': map(rule_to_str, result['stop_rules']),
        }
...
class FXData:
    ...
    def _check(args, against, required=True):
        ...
        '''
        res = {}
        for rarg, (rtype, rvals) in against.items():
            try:
                val = args.pop(rarg)
            except KeyError:
                if required:
    ...
    def jobResults(sid, field, sessionKey, count=-1):
        try:
            ...
            for res in job.results:
                if field in res:
                    i += 1
                    yield to_unicode(repr(res[field]))
                if i == count: break
...
#                    print('regex %s is a dup' % result['regex'])
    ...
    class MatchResult():
        ...
        def __init__(self, re_match):
            ...
            self.selectedText = groupdict.values()[0]
            self.selectedRange = re_match.span(groupdict.keys()[0])
...
#        self.timer.stop()
    ...
    def testRegex(self, field_val, selected_text, regex_start, regex_extract, regex_stop):
        ...
        try:
            ...
            print("regex %s doesn't match field value %s" %(user_regex.pattern, field_value))
            return False
        else:
    ...
    def testRules(self, data):
        data['field_value'] = to_unicode(data['field_value'])
        data['selected_text'] = to_unicode(data['selected_text'])
...
#              }
    ...
    def case33(self):
        ''' Testing the rule_to_str() function.
        '''
        rule = { # unicode values \
            'regex': u'^.*?Fran\xe7ais\=',
            'regex-preview': u'^(.*?)(Fran\xe7ais\=)',
            'type': u'<i>Fran\xe7ais=</i>'
        }
        rule_str = { \
            "regex-preview": "^(.*?)(Français\\=)", 
            "regex": "^.*?Français\\=", 
            "type": "<i>Français=</i>"
        }
        self.assertEqual(rule_to_str(rule), rule_str)
    ...
    def httprequest(self, data, method='GET'):
        ...
        try:
            if type(data) == str:
                ...
                response, content = self.http.request(self.url + '?' + urllib.urlencode(data), method=method)
