...
def correlogram0(data, start, end, n=0):
    ...
    for i in range(1,n+1):
        yield autocovariance0(data, start, end, mean, i)/var
...
class Datafeed:
    def __init__(self, data, start, end, step, missingValued=False):
        ...
        self.end = end
        self.step = step
        self.data_len = (self.end-self.start)/self.step
        self.cur = self.start
...
class LL0:
    ...
    def setData(self, datafeed):
        ...
        i = self.fcstart
        while datafeed.next()==None and (self.fc==None or self.fc[i]==None): 
            i += self.state_step
        datafeed.rewind()
    ...
    def updateAll(self, q):
        ...
        if self.fc[self.fcstart] != None:
            ...
            self.fc[self.fcstart] = a = self.datafeed.next()
        ...
        for x in self.datafeed:
            ...
            i += self.state_step
        sigma /= max(1, (i-b)/self.state_step)
        self.sigma = sigma
        self.nu = q*sigma
...
class LLP:
    ...
    def setModels(self):
        ...
        for i in range(self.period):
            if self.datafeed.getVal(i) == None:
                ...
                while j < self.data_len and self.datafeed.getVal(j) == None:
                    j += 1
                denom = j - i + 1
                right = self.datafeed.getVal(j)/denom
                if i > 0: left = self.datafeed.getVal(i-1)/denom
                else: left = right
                w1 = (denom-1)*left
                w2 = right
                for k in range(i,j):
        ...
        for i in range(self.period):
            ...
            model_forecast_len = diff/self.period
    ...
    def computeStates(self):
        ...
        for i in range(model1.first_forecast_index(), len(self.fc)):
            if model2 == None or model2.p[i] == None or model2.fc[i] == None:
                ...
                self.p[i] = 0
                self.fc[i] = (model1.fc[i]+model2.fc[i])/2
            else:
                K = model1.p[i]/(model1.p[i]+model2.p[i])
...
class LLT2(LL0):
    ...
    def setData(self, datafeed):
        ...
        if i < self.datafeed.getEnd() and self.datafeed.getVal(i) == None:
            ...
            if j < self.datafeed.getEnd():
                fillval = (self.datafeed.getVal(data_start) + self.datafeed.getVal(j))/2
            else:
    ...
    def llh(self,zeta,eta):
        ...
        self.datafeed.reset()
        pt1 = self.datafeed.next()  
        pt2 = self.datafeed.next()
        a = [2*pt2 - pt1, pt2 - pt1] 
    ...
    def updateAll(self, zeta, eta):
        ...
        self.epsilon = epsilon/len(self.datafeed)
...
#        a = [2*pt2 - pt1, pt2 - pt1]
        ...
        if len(self.datafeed) > 2:
            self.epsilon = epsilon/(len(self.datafeed) - 2)
...
class LLB:
    ...
    def llh(self, t1,t2,t3, datarange=200):
        ...
        if t1 == 0: return t2/2
        return (self.data_len-1)*log(t1) + t2/2
...
class SPTest(unittest.TestCase):
    ...
    def testPeriod(self):
        data = range(7) + [None,None,None] + range(7)*5
