...
class PersistentServerConnectionRequestPacket(object):
    ...
    def read(self, handle):
        ...
        if self.is_first():
            ...
            for i in range(0, command_pieces):
                self.command.append(PersistentServerConnectionRequestPacket._read_string(handle))
    ...
    def _read_number(handle):
        while True:
            ...
            if v != "":		# ignore empty lines before a count
                break
        try:
            n = int(v)
        except ValueError:
...
class PersistentServerConnectionPacketParser(object):
    ...
    def run(self):
        ...
        while True:
            ...
            if not in_packet.read(sys.stdin):
                break
