...
def make_splunkhome_path_helper(relpath):
    ...
    if len(shared_storage) > 0:
        for candidate in on_shared_storage:
            ...
            if splunk.clilib.cli_common.isWindows and not verify_path_prefix(relpath, candidate):
                break
...
class BundleInstaller:
    ...
    # Download a bundle using the given urllib2.Request object and write to a file.
...
class ServerCertValidator:
    def __init__(self, pol):
        self._cafile = pol._cafile
        self._sslCommonNameList = pol._sslCommonNameList
        self._sslmethod = SSL.TLSv1_2_METHOD
        self._cipherSuite = pol._cipherSuite
        self._cert_ok = True
        self._name_check_ok = False
        self._error = None

    def verify_cb(self, conn, cert, errnum, depth, ok):
        if not ok:
            self._cert_ok = False
            self._error = crypto.X509_verify_cert_error_string(errnum)
        if (depth == 0) and (self._sslCommonNameList is not None):
                sub_commonName = cert.get_subject().commonName
                allowedCommonNames = self._sslCommonNameList.split(',')
                for allowedCommonName in allowedCommonNames:
                        if allowedCommonName == sub_commonName:
                                self._name_check_ok = True
        return ok

    def validate_server_cert(self, url):
        # if common name is not specified, assume name check has passed
        if self._sslCommonNameList is None:
                self._name_check_ok = True
        ctx = SSL.Context(self._sslmethod)
        ...
        if self._cipherSuite != None:
            ctx.set_cipher_list(self._cipherSuite)
        ...
        if not self._cert_ok:
            raise splunk.RESTException(503, "SSL handshake failed. %s" % (self._error))
        if not self._name_check_ok:
            raise splunk.RESTException(503, "SSL handshake failed. common name does not match %s" % (self._error))
