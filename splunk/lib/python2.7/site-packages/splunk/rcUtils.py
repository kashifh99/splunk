...
from rcCmds import remote_cmds, GLOBAL_ARGS, GLOBAL_ACTIONS, GLOBAL_DEFAULTS
...
DISPLAY_CHARS = {
                  ...
                  'tail':displayMonitor,
                  'oneshot':displayOneshot,
                  'index':displayIndex,
                  'udp':displayUdp,
                  'tcp':displayTcp,
                  'saved-search':displaySavedsearch,
                  'forward-server':displayForwardServer,
                  'search:search':displaySyncSearch,
                  'jobs':displayJobs,
                  'search:dispatch':displaySyncSearch,
                  'distributed-search':displayDistribSearch,
                  'app':displayApp,
                  'auth-method':displayAuthMethod,
                  'role-mappings':displayRoleMappings,
                  'deployments':displayDeployment,
                  'distributed-search':displayDistSearch,
                  'help':displayHelp,
                }
...
def handleHelp(endpoint, restArgList):
   ...
   if not help_text:
      ...
      try:
         l = [remote_cmds[k]['_common']['help'][restArgList['cmdname']] for k in remote_cmds.keys() if k != '_common' if 'help' in remote_cmds[k]['_common'] if restArgList['cmdname'] in remote_cmds[k]['_common']['help']]
      except:
...
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def makeRestCall(cmd=None, restArgList=None, obj=None, sessionKey=None, entityName=None, search=None, count=None, offset=None, sort_key=None, sort_dir=None, dotSplunk=None, timeout=None):
   """
   ...
   elif cmd in WHITELIST.keys():
      if obj not in WHITELIST[cmd]:
         displayGenericError(cmd=cmd, obj=obj)
   ...
   #hack for removing all async jobs, to be removed when an EAI endpoint is provided to do this
     ...
     return handleRemoveJobsAll(sessionKey, namespace, owner)
      ...
      if type(lf_eai_id) == types.DictType:
         ...
         eai_id = extractLevel1Feed(serverContent=eai_id_serverContent, filter=[lf_eai_id['filter']])
      ...
      try:
         serverResponse, serverContent = simpleRequest(uri, sessionKey=sessionKey, getargs=getargs, postargs=postargs, method=method, timeout=timeout)
         ...
         displayResourceError(cmd=cmd, obj=obj, uri=uri, serverContent=e)
         return 
      except Exception as e:
         raise
