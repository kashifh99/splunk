...
class BaseOverlay(object):
        ...
        for columnName in self.columnNames:
            if columnName not in NOT_NUMBER_COLUMNS:
                ...
                for (row, column, v) in self._numberCells[columnName]:
                    ...
                    if v is not None:
                        numberCount += 1
                if numberCount < self._notNullCellCount[columnName] / 2:
    ...
    def getNumberCellsWithColor(self):
        return self._iter(self._numberCells.values())

    def _getAllNumberValues(self):
        values = []
        for list in self._numberCells.values():
...
TIME_RAW_FORMAT = '%Y-%m-%dT%H:%M:%S.%f%z'
...
class TableData(object):
    ...
    def addRowFromSearchResult(self, rowResult):
        ...
        values = []
        strippedRowResultFields = {k.strip(): v for k, v in rowResult.fields.items()}
...
class DivisibleTable(Table):
    ...
    def __init__(self, data, colWidths=None, rowHeights=None, style=None, repeatRows=0, repeatCols=0, splitByRow=1,
                 emptyTableAction=None, ident=None, hAlign=None, vAlign=None, normalizedData=0, cellStyles=None):
        ...
        Table.__init__(self, data, colWidths, rowHeights, style, repeatRows, repeatCols, splitByRow, emptyTableAction,
                       ident, hAlign, vAlign, normalizedData, cellStyles)
    ...
    def _splitByColumns(self, tables, availWidth):
        ...
        T0._appendCommandsBeforeCol(n, self._bkgrndcmds)
        T0._appendCommandsBeforeCol(n, self._spanCmds)
        T0._appendCommandsBeforeCol(n, self._nosplitCmds)

        if repeatCols:
            T1._appendCommandsAfterColWithRepeatColumns(n, repeatCols, lineCmds)
            T1._appendCommandsAfterColWithRepeatColumns(n, repeatCols, self._bkgrndcmds)
            T1._appendCommandsAfterColWithRepeatColumns(n, repeatCols, self._spanCmds)
            T1._appendCommandsAfterColWithRepeatColumns(n, repeatCols, self._nosplitCmds)
        else:
            T1._appendCommandsAfterCol(n, lineCmds)
            T1._appendCommandsAfterCol(n, self._bkgrndcmds)
            T1._appendCommandsAfterCol(n, self._spanCmds)
            T1._appendCommandsAfterCol(n, self._nosplitCmds)
...
class TableBuilder(object):
    ...
    def __init__(self, tableData, fontManager, title=None, columnVAlignments=None, displayLineNumbers=False,
        ...
        self._maxColWidth = (self._tableSize[0] - self._ROW_NUMBER_COL_WIDTH) / 2 - 5
        self._fontManager = fontManager
        ...
        for rowIdx, row in enumerate(rawData):
            ...
            for cellNum, cell in enumerate(row):
                ...
                if "##__SPARKLINE__##" in cellValue:
                    ...
                    logger.debug("renderTable> create sparkline with options %s" % options)
