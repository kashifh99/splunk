...
from pdfgen_search import InlineSearchManager, SavedSearchManager, PostProcessSearchManager
...
def getDashboardTitleAndPanelsFromXml(dashboardXml, namespace, owner, sessionKey):
    dashboard = createDashboardFromXml(et.fromstring(unicode(dashboardXml).encode('utf-8')), sourceApp=namespace)
    panels = []
...
def extractHtmlContent(namespace, element):
    try:
        if 'rawcontent' in element.options and element.options['rawcontent']:
            element.options['rawcontent'] = tostring(fromstring(cleanHtmlMarkup(element.options['rawcontent'])), encoding='utf-8',method='xml')
            ...
            element.options['rawcontent'] = tostring(fromstring(output), encoding='utf-8', method='xml')
...
class DashboardPanel(AbstractViewType):
    ...
    def __init__(self, panelDict, search, sequenceNum, namespace, owner, sessionKey):
        ...
        if search is None or not search.resolve():
    ...
    def getSingleValueProps(self):
        ...
        if 'options' in self._panelDict:
            ...
            for key, value in singleValueOptions.iteritems():
