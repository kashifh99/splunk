...
def displayResourceError(**kwargs):
   ...
   if kwargs['obj'] in ['monitor', 'tail']:
      ...
      while path != urllib.unquote(path):
...
def extractLevel1Feed(**kwargs):
   ...
   atom = rest.format.parseFeedDocument(kwargs['serverContent'])
   if isinstance(atom, AtomEntry):
      d = nodeToPrimitive(atom.rawcontents)
   elif isinstance(atom, AtomFeed):
      d = nodeToPrimitive(atom[0].rawcontents)
...
def extractTitleFeed(link_tuple_list, **kwargs):
   ...
   if link_tuple_list:
      ...
      serverResponse, serverContent = simpleRequest(link_tuple_list[0][1], sessionKey=kwargs['sessionKey'],  method='GET')
